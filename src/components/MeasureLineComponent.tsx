...
interface MeasureLineComponentProps {
  line: MeasureLine;
  onBeatClick?: (measureId: string, beatPosition: 1 | 2 | 3 | 4, event: React.MouseEvent) => void;
  onBeatRightClick?: (measureId: string, beatPosition: 1 | 2 | 3 | 4, event: React.MouseEvent) => void;
  onExpandMeasure?: (measureId: string) => void;
...
const MeasureLineComponent: React.FC<MeasureLineComponentProps> = ({
  ...
  const handleBeatClick = (measureId: string, beatPosition: 1 | 2 | 3 | 4, event: React.MouseEvent) => {
    onBeatClick?.(measureId, beatPosition, event);
  };

  const handleBeatRightClick = (measureId: string, beatPosition: 1 | 2 | 3 | 4, event: React.MouseEvent) => {
    onBeatRightClick?.(measureId, beatPosition, event);
  };

  const handleExpandMeasure = (measureId: string) => {
    onExpandMeasure?.(measureId);
  };
      ...
      {line.measures.map((measure, index) => {
        ...
        return (
          <Measure
            ...
            onExpandClick={() => handleExpandMeasure(measure.id)}
